...
if set_option == "1세트 (사회적 촉진/억제)":
    ...
    with tab1:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("위 요약표를 바탕으로 빈칸 (ㄱ), (ㄴ), (ㄷ)에 들어갈 적절한 내용을 내용을 찾아 쓰시오.")
        ...
        ans_a = st.text_input("(ㄱ) 입력란:", placeholder="예: 비교적 쉬운 취미 생활이나 큰 노력을 들일 필요가 없는 과제")
        ans_b = st.text_input("(ㄴ) 입력란:", placeholder="예: 충분히 연습하며 익숙해질 때까지 차분하게 혼자 집중하는 시간")
        ans_c = st.text_input("(ㄷ) 입력란:", placeholder="예: 사회적 억제")
    ...
    with tab2:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("주어진 첫 문장에 이어지는 내용을 아래 조건에 맞추어 작성하여 한 편의 설명문을 완성하시오.")
        ...
        method1 = st.selectbox("첫 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "예시", "대조"])
        sent1 = st.text_area("첫 번째 문장 작성:")
        
        method2 = st.selectbox("두 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "예시", "대조"])
        sent2 = st.text_area("두 번째 문장 작성:")
    ...
    with tab3:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("장면 2에 들어갈 시각 요소와 청각 요소를 기획하고, 그렇게 연출했을 때 얻을 수 있는 효과를 설명하시오.")
        ...
        v_idx = st.text_area("시각 요소 계획 및 연출 효과 서술란:")
        a_idx = st.text_area("청각 요소 계획 및 연출 효과 서술란:")
...
# -------------------------------------------------------------------------
    ...
    with tab1:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("본문의 비유적 설명을 바탕으로 빈칸 (ㄱ), (ㄴ), (ㄷ)에 들어갈 핵심 단어나 문장을 완성하시오.")
        ...
        ans_a = st.text_input("(ㄱ) 입력란:", placeholder="예: 높은 곳에 고여 있는 물")
        ans_b = st.text_input("(ㄴ) 입력란:", placeholder="예: 전하가 이동하지 않고 머물러 있음")
        ans_c = st.text_input("(ㄷ) 입력란:", placeholder="예: 위험하지 않음")
    ...
    with tab2:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("주어진 첫 문장에 이어 정전기의 개념과 비유적 특성이 나타나도록 설명문을 완성하시오.")
        ...
        method1 = st.selectbox("첫 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "정의", "비교와 대조"])
        sent1 = st.text_area("첫 번째 문장 작성:")
        
        method2 = st.selectbox("두 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "정의", "비교와 대조"])
        sent2 = st.text_area("두 번째 문장 작성:")
    ...
    with tab3:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("장면 2의 구체적인 연출 계획과 그 연출을 통해 달성하려는 시청자 전달 효과를 서술하시오.")
        ...
        v_idx = st.text_area("시각 요소 및 효과 서술란:")
        a_idx = st.text_area("청각 요소 및 효과 서술란:")
...
# -------------------------------------------------------------------------
    ...
    with tab1:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("지문의 핵심 대조 항목을 고려하여 빈칸 (ㄱ), (ㄴ), (ㄷ)를 알맞게 채우시오.")
        ...
        ans_a = st.text_input("(ㄱ) 입력란:", placeholder="예: 로봇이 한 번의 실수 없이 완벽하게 피겨 스케이팅을 해내는 경기")
        ans_b = st.text_input("(ㄴ) 입력란:", placeholder="예: 감정이 없고 독자적인 철학이 없으므로 예술이 아니다")
        ans_c = st.text_input("(ㄷ) 입력란:", placeholder="예: 미술계 변화를 가져오고 예술의 범주를 확장하는 상징적 가치")
    ...
    with tab2:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("주어진 첫 문장에 이어 본문의 분석 및 대조 정보를 활용하여 설명 글을 완성하시오.")
        ...
        method1 = st.selectbox("첫 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "분석", "대조"])
        sent1 = st.text_area("첫 번째 문장 작성:")
        
        method2 = st.selectbox("두 번째 문장에 사용한 설명 방법 선택", ["선택 안 함", "분석", "대조"])
        sent2 = st.text_area("두 번째 문장 작성:")
    ...
    with tab3:
        ...
        # 발문 (바탕 텍스트)
        st.markdown("장면 2에 들어갈 구체적인 연출 매체 언어와 이를 통해 감상자에게 전달하고자 하는 의도를 서술하시오.")
